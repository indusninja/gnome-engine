// Tile.h : Class to define a tile on a level and its properties.
...
public:
	...
	std::map<int, Point3> vertices;
	std::map<int, Vector3> edges;
	Vector3 normal;
	std::map<int, Vector3> edgeNormals;
	std::map<int, int> neighbours;

	void initCollisionDetectionEdges(void);
	void draw(float colorR, float colorG, float colorB, UINT textureID);
};
