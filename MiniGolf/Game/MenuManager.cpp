...
void MenuManager::init(string courseName, vector<string> holes)
{
	Button btn;
...
void MenuManager::ResetScores(void)
{
	mTotalScore = 0;
	ResetHoleStrokes();
}

void MenuManager::RearrangeMenus(void)
{
	for(map<int, Menu>::iterator i=mMenus.begin();i!=mMenus.end();i++)
		i->second.ArrangeButtons();
}

void MenuManager::setActiveMenu(int index)
{
...
int MenuManager::getActiveMenu(void)
{
	return activeMenuIndex;
}

void MenuManager::updatePower(void)
{
	Power += powerStep;
	if(Power>=20)
		powerStep = -1;
...
int MenuManager::getPower(void)
{
	return Power;
}

void MenuManager::incrementStroke(void)
{
	mHoleScore++;
	//mHolePar = par;
	mTotalScore++;
...
void MenuManager::ResetHoleStrokes(void)
{
	mHoleScore = 0;
}

void MenuManager::SetHolePar(int par)
{
	mHolePar = par;
}

// Take actions for the buttons selected
int MenuManager::CheckClick(int x, int y)
{
...
void MenuManager::Draw(bool stationary, bool playCurrentHoleOnly)
{
	if(isActive)
...
void MenuManager::DrawHUD(bool stationary, bool playCurrentHoleOnly)
{
