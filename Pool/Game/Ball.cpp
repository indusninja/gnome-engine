...
// specified normal
void Ball::init(int ballOnTileID, Point3 vertex, Vector3 normal)
{
	memset(this, 0, sizeof(Ball));
	onTileID = ballOnTileID;
	velocity.Zero();
	acceleration.Zero();
...
void Ball::draw(float colorR, float colorG, float colorB, string textureName)
{
	glPushMatrix();
